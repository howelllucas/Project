...
namespace EZ.Weapon
{
    public class EleCrossbowGun : Gun
    {
        ...
        {
            ItemItem weaponItem = Global.gApp.gGameData.GetItemDataByName(GameConstVal.WElecrossbowGun);
            ...
            int gunLevel = weaponMgr.GetWeaponLevel(GameConstVal.WElecrossbowGun);
            ...
            double atk = weaponLevelData.weapon_elecrossbow[(int)MWeapon.Atk];
            if (GetRealQuality(weaponItem) > 0)
            {
                atk = weaponLevelData.weapon_elecrossbow_super[(int)MWeapon.Atk];
            }
            SetDamage(atk);
        }
