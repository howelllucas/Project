...
namespace EZ.Weapon
{
    public class TornadoGun : Gun
    {
        ...
        {
            ItemItem weaponItem = Global.gApp.gGameData.GetItemDataByName(GameConstVal.WTornadoGun);
            ...
            int gunLevel = weaponMgr.GetWeaponLevel(GameConstVal.WTornadoGun);
            ...
            double atk = weaponLevelData.weapon_tornado[(int)MWeapon.Atk];
            if (GetRealQuality(weaponItem) > 0)
            {
                atk = weaponLevelData.weapon_tornado_super[(int)MWeapon.Atk];
            }
            SetDamage(atk);
        }
