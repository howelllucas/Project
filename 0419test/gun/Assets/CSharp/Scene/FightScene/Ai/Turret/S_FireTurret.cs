...
namespace EZ
{
    public class S_FireTurret : BaseTurret
    {
        ...
        [SerializeField]private bool m_Borned = false;
        private void Awake()
        {
            ...
            if (m_Borned)
            {
                ...
                gameObject.AddComponent<DelayCallBack>().SetAction(() => { SetChildActive(true);
                    PlayTurretAnim(m_TurretAnim, true);
                }, 0.1f);
            }
            else
            {
                m_AtkRange.gameObject.SetActive(m_Borned);
            }
        }
        ...
        void TurretEnded()
        {
            PlayTurretAnim(GameConstVal.Death);
            if (m_AnimTurretBase != null)
            {
                m_AnimTurretBase.enabled = true;
            }
            m_Gun.enabled = false;
            m_Borned = false;
        }
