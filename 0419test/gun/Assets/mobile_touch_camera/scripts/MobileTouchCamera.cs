...
#pragma warning restore 649
        ...
        public float GetCurrentTileAngleDeg()
        {
            return GetCurrentTiltAngleDeg(GetTiltRotationAxis());
        }
        #endregion
...
#pragma warning restore 162
        ...
        public void OnDrawGizmosSelected()
        {
            ...
            if (Editor__GetGameViewSize(out gameViewPixelWidth, out gameViewPixelHeight, out gameViewAspect))
            {
                ...
                //Fetch camera boundary as world-space coordinates projected to the ground.
                Vector2 camRight = GetIntersection2d(Cam.ScreenPointToRay(new Vector3(gameViewPixelWidth, gameViewPixelHeight * 0.5f, 0)));
                Vector2 camLeft = GetIntersection2d(Cam.ScreenPointToRay(new Vector3(0, gameViewPixelHeight * 0.5f, 0)));
                Vector2 camUp = GetIntersection2d(Cam.ScreenPointToRay(new Vector3(gameViewPixelWidth * 0.5f, gameViewPixelHeight, 0)));
                Vector2 camDown = GetIntersection2d(Cam.ScreenPointToRay(new Vector3(gameViewPixelWidth * 0.5f, 0, 0)));
                camProjectedMin = GetVector2Min(camRight, camLeft, camUp, camDown);
                camProjectedMax = GetVector2Max(camRight, camLeft, camUp, camDown);

                Gizmos.DrawLine(new Vector3(camProjectedMin.x, camProjectedMin.y), new Vector3(camProjectedMin.x, camProjectedMax.y));
                Gizmos.DrawLine(new Vector3(camProjectedMin.x, camProjectedMax.y), new Vector3(camProjectedMax.x, camProjectedMax.y));
                Gizmos.DrawLine(new Vector3(camProjectedMax.x, camProjectedMax.y), new Vector3(camProjectedMax.x, camProjectedMin.y));
                Gizmos.DrawLine(new Vector3(camProjectedMax.x, camProjectedMin.y), new Vector3(camProjectedMin.x, camProjectedMin.y));
            }
#endif
        }

#if UNITY_EDITOR
        private void InitByEditor()
        {
            if (Cam == null)
            {
                if (cameraTransform != null)
        ...
        public string EditorGetStateDataStr()
        {
            if (Application.isPlaying)
                return "";
            InitByEditor();
            var angle = GetCurrentTiltAngleDeg(GetTiltRotationAxis());
            float zoom;
            GetEditorCamZoom(out zoom);
            return string.Format("[zoom = {0:f2}, angle = {1:f2}]", zoom, angle);
        }

        public void FixByTiltRotation()
        {
            if (EnableZoomTilt == true)
            {
                float zoom;
                if(GetEditorCamZoom(out zoom))
                {
                    UpdateTiltForAutoTilt(zoom);
                }
            }
        }

        private bool GetEditorCamZoom(out float zoom)
        {
            if (Cam.orthographic == true)
            {
            ...
            else
            {
                if (IsTranslationZoom == true)
                {
                    ...
                    if (Editor__GetGameViewSize(out gameViewPixelWidth, out gameViewPixelHeight, out gameViewAspect))
                    {
                        Vector3 camCenterIntersection = GetIntersectionPoint(Cam.ScreenPointToRay(new Vector3(gameViewPixelWidth * 0.5f, gameViewPixelHeight * 0.5f, 0)));
                        zoom = (Vector3.Distance(camCenterIntersection, Transform.position));
                    }
                    else
                    {
                        zoom = float.NaN;
