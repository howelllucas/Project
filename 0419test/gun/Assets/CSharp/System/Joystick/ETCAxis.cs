...
public class ETCAxis {
	...
	public void UpdateButton(){
		...
		if (isValueOverTime){
			axisValue += overTimeStep * BaseScene.GetDtTime();
			axisValue = Mathf.Clamp( axisValue,0,maxOverTimeValue);
		}
		else{
		...
		switch (actionOn){
			case ActionOn.Down:
				...
				break;
			case ActionOn.Press:
				axisSpeedValue = axisValue * speed * BaseScene.GetDtTime();
			...
			case ETCAxis.DirectAction.Translate:
				if ( directCharacterController==null){
					...
					if (directCharacterController.isGrounded || !isLockinJump){
						...
						lastMove = localAxis *  (axisSpeedValue/BaseScene.GetDtTime());
					}
					else{
						directCharacterController.Move( lastMove * BaseScene.GetDtTime());
					}
				}
				break;
			...
			case ETCAxis.DirectAction.TranslateLocal:
				if ( directCharacterController==null){
					...
					if (directCharacterController.isGrounded || !isLockinJump){
						...
						directCharacterController.Move( direction );
						lastMove =directCharacterController.transform.TransformDirection(localAxis) *  (axisSpeedValue/BaseScene.GetDtTime());
					}
					else{
						directCharacterController.Move( lastMove * BaseScene.GetDtTime() );
					}
	...
	public void DoGravity(){
		...
		if (directCharacterController != null && gravity!=0){
			...
			if (!isJump){
				...
				directCharacterController.Move( move * BaseScene.GetDtTime());
			}
			else{
				currentGravity -= gravity*BaseScene.GetDtTime();
	...
	private void DoAutoStabilisation(){
		...
		if (angle > startAngle - autoStabThreshold || angle < startAngle + autoStabThreshold){
			...
			if (angle > startAngle - autoStabThreshold){
				axis = angle + autoStabSpeed/100f*Mathf.Abs (angle-startAngle) * BaseScene.GetDtTime()*-1;
			}
			
			
			if (angle < startAngle + autoStabThreshold){
				axis = angle + autoStabSpeed/100f*Mathf.Abs (angle-startAngle) * BaseScene.GetDtTime();
			}
