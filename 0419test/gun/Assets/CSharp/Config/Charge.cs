...
namespace EZ.Data {
	...
	public partial class Charge : ScriptableObject {

		[SerializeField, HideInInspector]
		private ChargeItem[] _Items;
		public ChargeItem[] items { get { return _Items; } }

		public ChargeItem Get(string id) {
			int min = 0;
			int max = items.Length;
			while (min < max) {
				int index = (min + max) >> 1;
				ChargeItem item = _Items[index];
	...
	public class ChargeItem {
		...
		public override string ToString() {
			return string.Format("[ChargeItem]{{id:{0}, goodsType:{1}, goodsNum:{2}, desc:{3}, itemIcon:{4}, consumeType:{5}, price:{6}, type:{7}, platform:{8}, effectNum:{9}, bgLight:{10}}}",
