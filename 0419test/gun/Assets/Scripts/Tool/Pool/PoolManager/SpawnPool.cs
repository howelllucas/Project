...
namespace Game.Pool
{
        ...
        public Transform Spawn(Transform prefab, Vector3 pos, Quaternion rot, Transform parent)
        {
            ...
            for (int i = 0; i < this._prefabPools.Count; i++)
            {
                ...
                if (this._prefabPools[i].prefabGO == prefab.gameObject)
                {
                    ...
                    #region 修改代码
                    PoolItem poolItem = inst.GetComponent<PoolItem>();
                    if (poolItem != null)
                    {
                        poolItem.OnSpawn(this);
                    }
                    #endregion
            ...
            #region 修改代码
            PoolItem poolItem2 = inst.GetComponent<PoolItem>();
            if (poolItem2 != null)
            {
                poolItem2.OnSpawn(this);
            }
            #endregion
    ...
    public class PrefabPool
    {
        ...
        internal bool DespawnInstance(Transform xform, bool sendEventMessage)
        {
            ...
            #region 修改代码
            PoolItem poolItem = xform.GetComponent<PoolItem>();
            if (poolItem == null || !poolItem.disabledItem)
            {
                ...
                //xform.SetParent(this.spawnPool.dontReparent ? null : this.spawnPool.group, false);
            }
            else
            {
    ...
    public class PrefabsDict : IDictionary<string, Transform>
    {
        ...
        public Transform this[string key]
        {
            get
            {
                Transform prefab;
                #region 修改代码
                if (this._prefabs != null && this._prefabs.ContainsKey(key))
                {
