...
namespace EZ.Weapon
{
    public class FireCrossbowGun : Gun
    {
        ...
        {
            ItemItem weaponItem = Global.gApp.gGameData.GetItemDataByName(GameConstVal.WFirecrossbowGun);
            ...
            int gunLevel = weaponMgr.GetWeaponLevel(GameConstVal.WFirecrossbowGun);
            ...
            double atk = weaponLevelData.weapon_firecrossbow[(int)MWeapon.Atk];
            if (GetRealQuality(weaponItem) > 0)
            {
                atk = weaponLevelData.weapon_firecrossbow_super[(int)MWeapon.Atk];
            }
            SetDamage(atk);
        }
