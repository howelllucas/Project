...
namespace Pathfinding {
	...
	public abstract class AIBase : VersionedMonoBehaviour {
        ...
        private bool CanFixedUpdate = true;
		...
		public virtual void Teleport (Vector3 newPosition, bool clearPath = true) {
			...
			if (rvoController != null) rvoController.Move(rvoController.velocity);
		...
		protected virtual void FixedUpdate () {
			if (!(rigid == null && rigid2D == null) && canMove && CanFixedUpdate) {
		...
		void FinalizeRotation (Quaternion nextRotation) {
			...
			if (updateRotation) {
                if (rigid != null) rigid.MoveRotation(nextRotation);
                    ...
                    rigid2D.transform.eulerAngles = new Vector3(0, 0, nextRotation.eulerAngles.z); // ³Â¶¬ÐÞ¸Ä
                    //Debug.Log("is2D" + nextRotation.eulerAngles.z);
                }
		...
		void FinalizePosition (Vector3 nextPosition) {
			...
			if ((positionDirty1 || positionDirty2) && updatePosition) {
                ...
                else if (rigid2D != null)
                {
                    ...
                    {
                        rigid2D.transform.position = currentPosition;
                    } else
                    {
