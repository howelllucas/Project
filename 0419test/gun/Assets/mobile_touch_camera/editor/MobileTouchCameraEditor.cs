    ...
    public override void OnInspectorGUI() {
      ...
      MobileTouchCamera mobileTouchCamera = (MobileTouchCamera)target;
            ...
            if (!Application.isPlaying)
            {
                ...
                {
                    if (GUILayout.Button("Normalize Root"))
                    {
                        NormalizeRoot(mobileTouchCamera);
                    }
                    if (GUILayout.Button("Reset Root"))
                    {
                        ResetRoot(mobileTouchCamera);
                    }
                }
                GUILayout.EndHorizontal();
                if (GUILayout.Button("Normalize Trans"))
                {
                    mobileTouchCamera.FixByTiltRotation();
                    EditorUtility.SetDirty(mobileTouchCamera.transform);
                }
                GUILayout.Label(mobileTouchCamera.EditorGetStateDataStr());
            }
