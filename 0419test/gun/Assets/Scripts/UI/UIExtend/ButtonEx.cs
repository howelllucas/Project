...
namespace UnityEngine.UI
{
    ...
    public class ButtonEx : Selectable, IPointerClickHandler, ISubmitHandler
    {
        ...
        public float longPressInvokeInterval = 0.1f;
        [FormerlySerializedAs("onLongPress")]
        [SerializeField]
        // 长按
        public ButtonClickedEvent m_OnLongPress = new ButtonClickedEvent();
        ...
        // 长按需要的变量参数
        private bool isStartPress = false;
        private float curPointDownTime = 0f;
        private float longPressTime = 0.6f;
        private float lastPressInvokeTime = 0f;
        ...
        void CheckIsLongPress()
        {
            if (isStartPress)
            {
                if (!longPressTrigger)
                {
                    if (Time.realtimeSinceStartup >= curPointDownTime + longPressTime)
                    {
                        longPressTrigger = true;
                        lastPressInvokeTime = Time.realtimeSinceStartup - longPressInvokeInterval;
                    }
                }
                else
                {
                    if (Time.realtimeSinceStartup >= lastPressInvokeTime + longPressInvokeInterval)
                    {
                        m_OnLongPress?.Invoke();
                        lastPressInvokeTime = Time.realtimeSinceStartup;
                        Debug.Log("LongPress");
                    }
                }
            }
        ...
        public override void OnPointerDown(PointerEventData eventData)
        {
            ...
            curPointDownTime = Time.realtimeSinceStartup;
            isStartPress = true;
