...
namespace EZ.Weapon
{
    public class IceCrossbowGun : Gun
    {
        ...
        {
            ItemItem weaponItem = Global.gApp.gGameData.GetItemDataByName(GameConstVal.WIcecrossbowGun);
            ...
            int gunLevel = weaponMgr.GetWeaponLevel(GameConstVal.WIcecrossbowGun);
            ...
            double atk = weaponLevelData.weapon_icecrossbow[(int)MWeapon.Atk];
            if (GetRealQuality(weaponItem) > 0)
            {
                atk = weaponLevelData.weapon_icecrossbow_super[(int)MWeapon.Atk];
            }
            SetDamage(atk);
        }
